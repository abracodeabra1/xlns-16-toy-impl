...
// (these are also set via CMake target_compile_definitions, but
//  we guard here for clarity)
#ifndef xlns16_table
#define xlns16_table
#endif
#ifndef xlns16_alt
...
void lns_diag_mask_inf(struct ggml_tensor * dst) {
    ...
    if (!inplace) {
        ...
        GGML_ASSERT(ggml_nbytes(dst) <= ggml_nbytes(src0) * 2); // safety: dst buffer was F32-sized
        memcpy(dst->data, src0->data, ggml_nbytes(dst));
    }

    if (dst->type == GGML_TYPE_LNS16) {
        const xlns16 neg_inf = fp2xlns16(-INFINITY);
